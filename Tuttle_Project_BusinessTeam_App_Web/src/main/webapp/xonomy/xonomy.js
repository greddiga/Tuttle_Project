...
Xonomy.verifyDocSpecElement = function(name) { // make sure the DocSpec object
	// has such an element, that the
	// element has everything it
	// needs
	if (!Xonomy.docSpec.elements[name]
...
Xonomy.refresh = function() {
	$(".xonomy .children ").each(
			function() { // determine whether each element does or doesn't
				// have children:
				if (this.childNodes.length == 0
						&& !$(this.parentNode).hasClass("hasText"))
	...
	$(".xonomy .element ")
			.each(
					function() { // reorder elements if necessary
						var elSpec = Xonomy.docSpec.elements[this
								.getAttribute("data-name")];
						if (elSpec.mustBeBefore) { // is it after an element it
							// cannot be after? then
							// move it up until it's
							// not!
							var $this = $(this);
							var ok;
							do {
						...
						if (elSpec.mustBeAfter) { // is it before an element
							// it cannot be before? then
							// move it down until it's
							// not!
							var $this = $(this);
							var ok;
							do {
	...
	$(".xonomy .attribute ").each(
			function() { // reorder attributes if necessary
				var atName = this.getAttribute("data-name");
				var elName = this.parentNode.parentNode.parentNode
						.getAttribute("data-name");
				...
				if (mustBeBefore.length > 0) { // is it after an attribute it
					// cannot be after? then move it
					// up until it's not!
					var $this = $(this);
					var ok;
					do {
				...
				if (mustBeAfter.length > 0) { // is it before an attribute it
					// cannot be before? then move
					// it down until it's not!
					var $this = $(this);
					var ok;
					do {
						ok = true;
...
Xonomy.harvestElement = function(htmlElement, jsParent) {
	...
	js.attributes = [];
	var htmlAttributes = $(htmlElement).find(".tag.opening > .attributes")
			.toArray()[0];
...
Xonomy.render = function(data, editor, docSpec) { // renders the contents of
	// an editor
	// The data can be a Xonomy-compliant XML document, a Xonomy-compliant
	// xml-as-string,
	...
	// Convert doc to a JavaScript object, if it isn't a JavaScript object
	// already:
...
Xonomy.renderElement = function(element) {
	...
	if (spec.menu.length > 0)
		classNames += " hasMenu"; // not always accurate: whether an element
	// has a menu is actually determined at
	// runtime
	var displayName = element.name;
	if (spec.displayName)
		displayName = "<span class='displayName'>"
				+ Xonomy.textByLang(spec.displayName) + "</span>";
	var html = "";
	html += '<div data-name="' + element.name + '" id="' + htmlID + '" class="'
			+ classNames + '">';
	html += '<span class="connector">';
	html += '<span class="plusminus" onclick="Xonomy.plusminus(\'' + htmlID
			+ '\')"></span>';
	html += '<span class="draghandle" draggable="true" ondragstart="Xonomy.drag(event)"></span>';
	html += '</span>';
	html += '<span class="tag opening" style="background-color: '
			+ spec.backgroundColour + ';">';
	html += '<span class="punc">&lt;</span>';
	html += '<span class="name" onclick="Xonomy.click(\'' + htmlID
			+ '\', \'openingTagName\')">' + displayName + '</span>';
	html += '<span class="warner"><span class="inside" onclick="Xonomy.click(\''
			+ htmlID + '\', \'warner\')"></span></span>';
	...
	html += '<span class="punc">&gt;</span>';
	html += '</span>';
	html += '<span class="childrenCollapsed" onclick="Xonomy.plusminus(\''
			+ htmlID + '\', true)">&middot;&middot;&middot;</span>';
	...
	if (hasText && element.children.length > 1
	...
	html += '</div>';
	html += '<span class="tag closing" style="background-color: '
			+ spec.backgroundColour + ';">';
	html += '<span class="punc">&lt;</span>';
	html += '<span class="punc">/</span>';
	html += '<span class="name" onclick="Xonomy.click(\'' + htmlID
			+ '\', \'closingTagName\')">' + displayName + '</span>';
	html += '<span class="punc">&gt;</span>';
	html += '</span>';
	html += '</div>';
...
Xonomy.renderAttribute = function(attribute, optionalParentName) {
	...
	var html = "";
	html += '<span data-name="' + attribute.name + '" data-value="'
			+ Xonomy.xmlEscape(attribute.value) + '" id="' + htmlID
			+ '" class="' + classNames + '">';
	html += '<span class="punc"> </span>';
	var onclick = '';
	if (!readonly)
		onclick = ' onclick="Xonomy.click(\'' + htmlID
				+ '\', \'attributeName\')"';
	...
	html += '<span class="punc">"</span>';
	html += '<span class="value">' + Xonomy.xmlEscape(attribute.value)
			+ '</span>';
	html += '<span class="punc">"</span>';
	html += '</span>';
	html += '</span>';
...
Xonomy.renderText = function(text) {
	...
	if (text.value == "")
		classNames += " empty";
	var html = "";
	html += '<div id="' + htmlID + '" data-value="'
			+ Xonomy.xmlEscape(text.value) + '" class="' + classNames + '">';
	html += '<span class="connector"></span>';
	html += '<span class="value" onclick="Xonomy.click(\''
			+ htmlID
			+ '\', \'text\')"><span class="insertionPoint"><span class="inside"></span></span><span class="dots"></span>'
			+ Xonomy.chewText(text.value) + '</span>';
	html += '</div>';
	text.htmlID = htmlID;
	return html;
}
...
Xonomy.charClick = function(c) {
	...
	if ($(".xonomy .char.on").toArray().length == 1 && // if there is precisely
	// one previously
	// selected character
	$(".xonomy .char.on").closest(".element").is($(c).closest(".element")) // and
	// if
	// it
	// has
	// the
	...
	// character
		...
		for (var i = 0; i < chars.length; i++) { // highlight all chars
		...
		Xonomy.textFromIndex = $.inArray(chars[iFrom], $(
				"#" + Xonomy.textFromID).find(".char").toArray());
		Xonomy.textTillIndex = $.inArray(chars[iTill], $(
				"#" + Xonomy.textTillID).find(".char").toArray());
		// Show inline menu etc:
		var htmlID = $element.attr("id");
		var content = Xonomy.inlineMenu(htmlID); // compose bubble content
		if (content != "") {
			document.body.appendChild(Xonomy.makeBubble(content)); // create
			// bubble
			Xonomy.showBubble($("#" + htmlID + " .char.on").last()); // anchor
			// bubble
			// to
			// highlighted
			// chars
...
Xonomy.wrap = function(htmlID, param) {
	...
	if (Xonomy.textFromID == Xonomy.textTillID) { // abc --> a<XYZ>b</XYZ>c
		var jsOld = Xonomy.harvestText(document
				.getElementById(Xonomy.textFromID));
		...
		var jsOldOpen = Xonomy.harvestText(document
				.getElementById(Xonomy.textFromID));
		var jsOldClose = Xonomy.harvestText(document
				.getElementById(Xonomy.textTillID));
		...
		var txtMiddleClose = jsOldClose.value.substring(0,
				Xonomy.textTillIndex + 1);
		var txtClose = jsOldClose.value.substring(Xonomy.textTillIndex + 1);
		xml = xml.replace(ph, Xonomy.xmlEscape(txtMiddleOpen) + ph);
		$("#" + Xonomy.textFromID).nextUntil("#" + Xonomy.textTillID).each(
				function() {
					if ($(this).hasClass("element"))
						xml = xml.replace(ph, Xonomy.js2xml(Xonomy
								.harvestElement(this))
								+ ph);
...
Xonomy.click = function(htmlID, what) {
	if (!Xonomy.notclick) {
		...
		if (what == "openingTagName" || what == "closingTagName") {
			...
			var content = Xonomy.elementMenu(htmlID); // compose bubble
			// content
			if (content != "") {
				document.body.appendChild(Xonomy.makeBubble(content)); // create
				// bubble
				if (what == "openingTagName")
					Xonomy.showBubble($("#" + htmlID
							+ " > .tag.opening > .name")); // anchor bubble to
				// opening tag
				if (what == "closingTagName")
					Xonomy.showBubble($("#" + htmlID
							+ " > .tag.closing > .name")); // anchor bubble to
				// closing tag
			}
		}
		if (what == "attributeName") {
			$("#" + htmlID).addClass("current"); // make the attribute
			// current
			var content = Xonomy.attributeMenu(htmlID); // compose bubble
			// content
			if (content != "") {
				document.body.appendChild(Xonomy.makeBubble(content)); // create
				// bubble
				Xonomy.showBubble($("#" + htmlID + " > .name")); // anchor
				// bubble to
				// attribute
				// name
			}
		...
		if (what == "attributeValue") {
			$("#" + htmlID + " > .valueContainer").addClass("current"); // make
			// attribute
			// value
			// current
			var name = $("#" + htmlID).attr("data-name"); // obtain
			// attribute's name
			var value = $("#" + htmlID).attr("data-value"); // obtain current
			// value
			...
			var content = spec.asker(value, spec.askerParameter); // compose
			// bubble
			// content
			if (content != "") {
				document.body.appendChild(Xonomy.makeBubble(content)); // create
				// bubble
				Xonomy.showBubble($("#" + htmlID
						+ " > .valueContainer > .value")); // anchor bubble to
				// value
		...
		if (what == "text") {
			...
			if (typeof (spec.asker) != "function") {
				var content = Xonomy.askLongString(value); // compose bubble
				// content
			} else {
				var content = spec.asker(value, spec.askerParameter); // use
				// specified
				// asker
			}
			document.body.appendChild(Xonomy.makeBubble(content)); // create
			// bubble
			Xonomy.showBubble($("#" + htmlID + " > .value")); // anchor bubble
			// to value
			Xonomy.answer = function(val) {
				var obj = document.getElementById(htmlID);
				var jsText = {
				...
				$(obj).replaceWith(html);
				Xonomy.clickoff();
				Xonomy.changed(Xonomy.harvestText(document
						.getElementById(jsText.htmlID)));
			};
		}
		if (what == "warner") {
			// $("#"+htmlID).addClass("current");
			...
			for (var iWarning = 0; iWarning < Xonomy.warnings.length; iWarning++) {
				...
				if (warning.htmlID == htmlID) {
					content += "<div class='warning'>"
							+ Xonomy.formatCaption(Xonomy
									.textByLang(warning.text)) + "</div>";
				}
			}
			document.body.appendChild(Xonomy.makeBubble(content)); // create
			// bubble
			Xonomy.showBubble($("#" + htmlID + " .warner .inside").first()); // anchor
			// bubble
			// to
			// warner
		}
...
Xonomy.clearChars = false; // if true, un-highlight any highlighted characters
// at the next click-off event
...
Xonomy.makeBubble = function(content) {
	...
	bubble.innerHTML = "<div class='inside' onclick='Xonomy.notclick=true;'>"
			+ "<div id='xonomyBubbleContent'>" + content + "</div>" + "</div>";
	return bubble;
};
Xonomy.showBubble = function($anchor) {
	...
	$bubble.slideDown("fast", function() {
		$bubble.find(".focusme").first().focus(); // if the context menu
		// contains anything with
		// the class name 'focusme',
		// focus it.
	});
};
...
Xonomy.askLongString = function(defaultString) {
	var html = "";
	html += "<form onsubmit='Xonomy.answer(this.val.value); return false'>";
	html += "<textarea name='val' class='textbox focusme' spellcheck='false'>"
...
Xonomy.askPicklist = function(defaultString, picklist) {
	...
	for (var i = 0; i < picklist.length; i++) {
		...
		html += "<div class='menuItem techno"
				+ (item.value == defaultString ? " current" : "")
				+ "' onclick='Xonomy.answer(\"" + Xonomy.xmlEscape(item.value)
				+ "\")'>";
		html += "<span class='punc'>\"</span>";
		html += Xonomy.xmlEscape(item.value);
		html += "<span class='punc'>\"</span>";
		if (item.caption != "")
			html += " <span class='explainer'>"
...
Xonomy.attributeMenu = function(htmlID) {
	...
	var elName = $("#" + htmlID).closest(".element").attr("data-name"); // obtain
	// element's
	// name
	Xonomy.verifyDocSpecAttribute(elName, name);
	var spec = Xonomy.docSpec.elements[elName].attributes[name];
	...
	for (var i = 0; i < spec.menu.length; i++) {
		...
		var includeIt = !item.hideIf(Xonomy.harvestAttribute(document
				.getElementById(htmlID)));
		if (includeIt) {
			html += "<div class='menuItem' onclick='Xonomy.callMenuFunction(Xonomy.docSpec.elements[\""
					+ elName
					+ "\"].attributes[\""
					+ name
					+ "\"].menu["
...
Xonomy.elementMenu = function(htmlID) {
	...
	for (var i = 0; i < spec.menu.length; i++) {
		var item = spec.menu[i];
		var includeIt = !item.hideIf(Xonomy.harvestElement(document
				.getElementById(htmlID)));
		if (includeIt) {
			html += "<div class='menuItem' onclick='Xonomy.callMenuFunction(Xonomy.docSpec.elements[\""
					+ elName + "\"].menu[" + i + "], \"" + htmlID + "\")'>";
			html += Xonomy.formatCaption(Xonomy.textByLang(item.caption));
			html += "</div>";
		}
	}
...
Xonomy.insertElements = function(htmlID, parameter) {
	for (var i = 0; i <= parameter.length; i++) {
		Xonomy.newElementChild(htmlID, parameter[i]);
	}
};
...
Xonomy.drag = function(ev) { // called when dragging starts
	...
	$(".xonomy .children .text")
			.before(
					"<div class='elementDropper' ondragover='Xonomy.dragOver(event)' ondragleave='Xonomy.dragOut(event)' ondrop='Xonomy.drop(event)'><div class='inside'></div></div>")
	$(".xonomy .dragging .elementDropper").remove(); // remove drop targets
	// inside the element
	// being dragged
	$(".xonomy .dragging").prev(".elementDropper").remove(); // remove drop
	// targets from
	// immediately
	// before the
	// element being
	// dragged
	$(".xonomy .dragging").next(".elementDropper").remove(); // remove drop
	// targets from
	// immediately
	// after the
	// element being
	// dragged
	if (elSpec.canDropTo) { // remove the drop target from elements it cannot be
		// dropped into
		var droppers = $(".xonomy .elementDropper").toArray();
		for (var i = 0; i < droppers.length; i++) {
			var dropper = droppers[i];
			var parentElementName = $(dropper.parentNode.parentNode).toArray()[0]
					.getAttribute("data-name");
	...
	if (elSpec.mustBeBefore) { // remove the drop target from after elements it
		// cannot be after
		var droppers = $(".xonomy .elementDropper").toArray();
		for (var i = 0; i < droppers.length; i++) {
			var dropper = droppers[i];
			for (var ii = 0; ii < elSpec.mustBeBefore.length; ii++) {
				if ($(dropper).prevAll(
						"*[data-name='" + elSpec.mustBeBefore[ii] + "']")
						.toArray().length > 0) {
					dropper.parentNode.removeChild(dropper);
				}
			}
	...
	if (elSpec.mustBeAfter) { // remove the drop target from before elements
		// it cannot be before
		var droppers = $(".xonomy .elementDropper").toArray();
		for (var i = 0; i < droppers.length; i++) {
			var dropper = droppers[i];
			for (var ii = 0; ii < elSpec.mustBeAfter.length; ii++) {
				if ($(dropper).nextAll(
						"*[data-name='" + elSpec.mustBeAfter[ii] + "']")
						.toArray().length > 0) {
					dropper.parentNode.removeChild(dropper);
				}
			}
...
Xonomy.drop = function(ev) {
	ev.preventDefault();
	var node = document.getElementById(Xonomy.draggingID); // the thing we are
	// moving
